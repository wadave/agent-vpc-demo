...
resource "google_cloudbuild_trigger" "cd_pipeline" {
  ...
  substitutions = {
    ...
    _FRONTEND_SERVICE_ACCOUNT_STAGING = google_service_account.frontend_sa["staging"].email
    _REGION                           = var.region
    _CONTAINER_NAME                   = var.project_name
    _ARTIFACT_REGISTRY_REPO_NAME      = resource.google_artifact_registry_repository.repo_artifacts.repository_id
  }
  depends_on = [
    resource.google_project_service.cicd_services,
    resource.google_project_service.deploy_project_services,
...
resource "google_cloudbuild_trigger" "deploy_to_prod_pipeline" {
  ...
  substitutions = {
    ...
    _FRONTEND_SERVICE_ACCOUNT_PROD = google_service_account.frontend_sa["prod"].email
    _REGION                        = var.region
    _CONTAINER_NAME                = var.project_name
    _ARTIFACT_REGISTRY_REPO_NAME   = resource.google_artifact_registry_repository.repo_artifacts.repository_id
  }
