...
# Grant frontend SA project-level roles
resource "google_project_iam_member" "frontend_sa_roles" {
  for_each = {
    for pair in setproduct(keys(local.project_ids), var.frontend_sa_roles) :
    join(",", pair) => {
      project = local.project_ids[pair[0]]
      role    = pair[1]
    }
...
# Grant frontend SA permission to invoke the backend
...
# Allow unauthenticated access to frontend (dev environment)
