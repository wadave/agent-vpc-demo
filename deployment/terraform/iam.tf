...
# 3.4. Grant frontend SA the required project-level roles
resource "google_project_iam_member" "frontend_sa_roles" {
  for_each = {
    for pair in setproduct(keys(local.deploy_project_ids), var.frontend_sa_roles) :
    join(",", pair) => {
      project = local.deploy_project_ids[pair[0]]
      role    = pair[1]
    }
  ...
  project    = each.value.project
  role       = each.value.role
  member     = "serviceAccount:${google_service_account.frontend_sa[split(",", each.key)[0]].email}"
  depends_on = [resource.google_project_service.cicd_services, resource.google_project_service.deploy_project_services]
}

# 3.5. Grant frontend SA the required permissions to invoke the backend
...
# 3.6. Allow unauthenticated access to frontend via the load balancer
